...
$items = \app\models\Circle::find()
    ->select(['name', 'id'])
    ->indexBy('id')
    ->column();

$item = \app\models\Category::find()
    ->select(['name', 'id'])
    ->indexBy('id')
    ->column();
    ...
    <?php else: ?>
        ...
        <div class="row">
            <div class="col-lg-5">
                ...
                <?= $form->field($model, 'circle_id')->dropDownList($items, ['promt' => 'Выбирите кружок']) ?>
                <?= $form->field($model, 'category_id')->dropDownList($item, ['promt' => 'Выбирите возрост']) ?>
